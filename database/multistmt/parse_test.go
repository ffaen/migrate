...
func TestParse(t *testing.T) {
	testCases := []struct {
		...
		{name: "two statements, with trailing delimiter", multiStmt: "statement one; statement two;", delimiter: ";",
			expected: []string{"statement one;", " statement two;"}, expectedErr: nil},
		{name: "two statements, with multichar delimiter", multiStmt: "statement one OK statement two OK", delimiter: "OK",
			expected: []string{"statement one OK", " statement two OK"}, expectedErr: nil},
	}

	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			stmts := make([]string, 0, len(tc.expected))
			err := multistmt.Parse(strings.NewReader(tc.multiStmt), []byte(tc.delimiter), maxMigrationSize, true, func(b []byte) bool {
...
func TestParseNoDelimiter(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			stmts := make([]string, 0, len(tc.expected))
			err := multistmt.Parse(strings.NewReader(tc.multiStmt), []byte(tc.delimiter), maxMigrationSize, false, func(b []byte) bool {
				stmts = append(stmts, string(b))
				return true
			})
			assert.Equal(t, tc.expectedErr, err)
...
func TestParseDiscontinue(t *testing.T) {
	...
	err := multistmt.Parse(strings.NewReader(multiStmt), []byte(delimiter), maxMigrationSize, true, func(b []byte) bool {
		stmts = append(stmts, string(b))
		return false
	})
	assert.Nil(t, err)
